...
uint8_t usx_sets[][28] = {{0, ' ', 'e', 't', 'a', 'o', 'i', 'n', 's', 'r', 'l', 'c', 'd', 'h', 'u', 'p', 'm', 'b', 'g',
                              'w', 'f', 'y', 'v', 'k', 'q', 'j', 'x', 'z'},
    {'"', '{', '}', '_', '<', '>', ':', '\n', 0, '[', ']', '\\', ';', '\'', '\t', '@', '*', '&', '?', '!', '^', '|',
        '\r', '~', '`', 0, 0, 0},
    {0, ',', '.', '0', '1', '9', '2', '5', '-', '/', '3', '4', '6', '7', '8', '(', ')', ' ', '=', '+', '$', '%', '#', 0,
        0, 0, 0, 0}};
...
uint8_t usx_vcodes[] = {0x00, 0x40, 0x60, 0x80, 0x90, 0xA0, 0xB0, 0xC0, 0xD0, 0xD8, 0xE0, 0xE4, 0xE8, 0xEC, 0xEE, 0xF0,
    0xF2, 0xF4, 0xF6, 0xF7, 0xF8, 0xF9, 0xFA, 0xFB, 0xFC, 0xFD, 0xFE, 0xFF};
...
int append_nibble_escape(
    char* out, int olen, int ol, uint8_t state, const uint8_t usx_hcodes[], const uint8_t usx_hcode_lens[]) {
  SAFE_APPEND_BITS(ol = append_switch_code(out, olen, ol, state));
  SAFE_APPEND_BITS(ol = append_bits(out, olen, ol, usx_hcodes[USX_NUM], usx_hcode_lens[USX_NUM]));
...
int append_final_bits(char* const out, const int olen, int ol, const uint8_t state, const uint8_t is_all_upper,
  ...
  SAFE_APPEND_BITS(ol = append_bits(out, olen, ol, (ol == 0 || out[(ol - 1) / 8] << ((ol - 1) & 7) >= 0) ? 0 : 0xFF,
                       (8 - ol % 8) & 7));
...
int unishox2_compress_lines(const char* in, int len, UNISHOX_API_OUT_AND_LEN(char* out, int olen),
    const uint8_t usx_hcodes[], const uint8_t usx_hcode_lens[], const char* usx_freq_seq[], const char* usx_templates[],
  ...
  for (l = 0; l < len; l++) {
    ...
    if (l < (len - 5) && usx_hcode_lens[USX_NUM]) {
      ...
      if ((hex_type == USX_NIB_HEX_LOWER || hex_type == USX_NIB_HEX_UPPER) && hex_len > 3) {
        ...
        SAFE_APPEND_BITS2(rawolen,
            ol = append_bits(
                out, olen, ol, (hex_type == USX_NIB_HEX_LOWER ? 0x80 : 0xE0), (hex_type == USX_NIB_HEX_LOWER ? 2 : 4)));
      ...
      for (i = 0; i < 6; i++) {
        ...
        if (len - seq_len >= 0 && l <= len - seq_len) {
          if (memcmp(usx_freq_seq[i], in + l, seq_len) == 0 && usx_hcode_lens[usx_freq_codes[i] >> 5]) {
            SAFE_APPEND_BITS2(
                rawolen, ol = append_code(out, olen, ol, usx_freq_codes[i], &state, usx_hcodes, usx_hcode_lens));
            l += seq_len;
            l--;
            break;
          }
    ...
    if (c_in >= 32 && c_in <= 126) {
      ...
      if (c_in == 0) {
        ...
        c_in--;
        SAFE_APPEND_BITS2(
            rawolen, ol = append_code(out, olen, ol, usx_code_94[(int)c_in], &state, usx_hcodes, usx_hcode_lens));
      }
...
int unishox2_compress(const char* in, int len, UNISHOX_API_OUT_AND_LEN(char* out, int olen), const uint8_t usx_hcodes[],
  ...
  return unishox2_compress_lines(
      in, len, UNISHOX_API_OUT_AND_LEN(out, olen), usx_hcodes, usx_hcode_lens, usx_freq_seq, usx_templates, NULL);
}

// Main API function. See unishox2.h for documentation
int unishox2_compress_simple(const char* in, int len, char* out) {
  auto HCODES = USX_HCODES_DFLT;
  auto HCODE_LENS = USX_HCODE_LENS_DFLT;
  const char* FREQ_SEQ[] = USX_FREQ_SEQ_DFLT;
  const char* TEMPLATES[] = USX_TEMPLATES;
  return unishox2_compress_lines(
      in, len, UNISHOX_API_OUT_AND_LEN(out, INT_MAX - 1), HCODES, HCODE_LENS, FREQ_SEQ, TEMPLATES, NULL);
}
...
uint8_t usx_vcode_lookup[36] = {(1 << 5) + 0, (1 << 5) + 0, (2 << 5) + 1, (2 << 5) + 2, // Section 1
    ...
    (5 << 5) + 12, (5 << 5) + 12, (6 << 5) + 13, (6 << 5) + 14, (6 << 5) + 15, (6 << 5) + 15, (6 << 5) + 16,
    (6 << 5) + 16, // Section 5
    (6 << 5) + 17, (6 << 5) + 17, (7 << 5) + 18, (7 << 5) + 19, (7 << 5) + 20, (7 << 5) + 21, (7 << 5) + 22,
    (7 << 5) + 23, (7 << 5) + 24, (7 << 5) + 25, (7 << 5) + 26, (7 << 5) + 27};
          ...
          if (idx == 0) {
            ...
            for (int j = 0; j < rem; j++) {
              ...
              if (c_t == 'f' || c_t == 'r' || c_t == 't' || c_t == 'o' || c_t == 'F') {
                ...
                DEC_OUTPUT_CHAR(
                    out, olen, ol++, getHexChar((char)raw_char, c_t == 'f' ? USX_NIB_HEX_LOWER : USX_NIB_HEX_UPPER));
                bit_no += nibble_len;
              } else
...
int unishox2_decompress(const char* in, int len, UNISHOX_API_OUT_AND_LEN(char* out, int olen),
  ...
  return unishox2_decompress_lines(
      in, len, UNISHOX_API_OUT_AND_LEN(out, olen), usx_hcodes, usx_hcode_lens, usx_freq_seq, usx_templates, NULL);
}
...
int unishox2_decompress_simple(const char* in, int len, char* out) {
  ...
  return unishox2_decompress(
      in, len, UNISHOX_API_OUT_AND_LEN(out, INT_MAX - 1), HCODES, HCODE_LENS, FREQ_SEQ, TEMPLATES);
}

int unishox2_compress_alnum(const char* in, int len, char* out) {
  auto HCODES = USX_HCODES_ALPHA_NUM_ONLY;
  auto HCODE_LENS = USX_HCODE_LENS_ALPHA_NUM_ONLY;
  const char* FREQ_SEQ[] = USX_FREQ_SEQ_DFLT;
  const char* TEMPLATES[] = USX_TEMPLATES;
  return unishox2_compress_lines(
      in, len, UNISHOX_API_OUT_AND_LEN(out, INT_MAX - 1), HCODES, HCODE_LENS, FREQ_SEQ, TEMPLATES, NULL);
...
int unishox2_decompress_alnum(const char* in, int len, char* out) {
  auto HCODES = USX_HCODES_ALPHA_NUM_ONLY;
  auto HCODE_LENS = USX_HCODE_LENS_ALPHA_NUM_ONLY;
  const char* FREQ_SEQ[] = USX_FREQ_SEQ_DFLT;
  const char* TEMPLATES[] = USX_TEMPLATES;
